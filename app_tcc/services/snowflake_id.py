...
def allocate_snowflake_int() -> int:
    url = (settings.TCC_SNOWFLAKE_ID_URL or "").strip()
    if not url:
        raise SnowflakeIdError("TCC_SNOWFLAKE_ID_URL is not configured")
    try:
        url = expand_service_url_from_env(url)
    except ServiceUrlResolutionError as e:
        raise SnowflakeIdError(str(e)) from e
    access_key = (settings.TCC_SNOWFLAKE_ACCESS_KEY or "").strip()
    if not access_key:
        raise SnowflakeIdError("TCC_SNOWFLAKE_ACCESS_KEY is not configured")
    timeout = float(settings.TCC_SNOWFLAKE_HTTP_TIMEOUT_SEC)
    try:
        resp = request_sync(
            method="POST",
            url=url,
        ...
        logger.warning("snowflake id request failed: %s", e)
