...
4. **配置**：`.env` / `settings` 读取 `DB_TCC_*`（默认库名 `sf_tcc`）、`TCC_SNOWFLAKE_ACCESS_KEY`；全局阶段超时、`TCC_MAX_AUTO_RETRIES`、出站 HTTP 超时等（首版不设 per-participant 超时）。

---

## P1 — 协调核心（Try / Confirm / Cancel 语义）

5. **雪花 ID**：通过 **`app_snowflake` HTTP** 生成全局事务 ID；客户端使用 `TCC_SNOWFLAKE_ACCESS_KEY`（`services/snowflake_id` 或等价分层）。
...
14. **单测**：Mock 参与者 HTTP 与 snowflake 客户端；覆盖：Try 全成功 → **逆序 Confirm**；Try 中途失败 → **仅已成功分支逆序 Cancel**；扫描重试与**并发 claim**。
