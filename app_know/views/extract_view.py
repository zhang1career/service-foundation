...
class ExtractBriefView(APIView):
    ...
    def post(self, request, point_id, *args, **kwargs):
        try:
            ...
            if not content:
                return resp_err(code=RET_INVALID_PARAM, message="Point content is empty")
            ...
            if not extracted:
                return resp_err(code=RET_INVALID_PARAM, message="Extract failed or AI unavailable")
...
class AnalyzeComponentsView(APIView):
    ...
    def post(self, request, point_id, *args, **kwargs):
        try:
            ...
            if not result:
                return resp_err(code=RET_INVALID_PARAM, message="Analyze failed or AI unavailable")
    ...
    def post(self, request, point_id, *args, **kwargs):
        try:
            ...
            if result.get("errors"):
                return resp_err(code=RET_INVALID_PARAM, message=implode(result["errors"], "; "))
...
class SentenceGraphView(APIView):
    ...
    def get(self, request, entity_id, *args, **kwargs):
        ...
        try:
            ...
            items, _ = list_by_batch(entity_id, limit=1)
            if not items:
                return resp_err(code=RET_RESOURCE_NOT_FOUND, message=f"Batch {entity_id} has no knowledge points")
...
class Neo4jCypherView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if result.get("error"):
                return resp_err(code=RET_INVALID_PARAM, message=result["error"])
    ...
    def post(self, request, point_id, *args, **kwargs):
        try:
            ...
            if not gs and not go:
                return resp_err(code=RET_INVALID_PARAM, message="成分关系为空，请先保存成分关系（graph_subject/graph_object）")
