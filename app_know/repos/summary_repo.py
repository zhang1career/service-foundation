...
def search_summaries_by_vector_filtered(
    ...
    """
    Search summaries by semantic similarity, return top_k.
    Used by knowledge list summary filter.
    Returns list of dicts with kid, summary, app_id, ct, ut, score (ordered by score desc).
    """
    logger.debug("[search_summaries_by_vector_filtered] query=%r, app_id=%s, top_k=%s", query[:80] if query else "", app_id, top_k)
    raw = search_summaries_by_vector(query=query, app_id=app_id, limit=top_k)
    logger.debug("[search_summaries_by_vector_filtered] count=%s, scores=%s", len(raw), [round(r.get("score", 0), 3) for r in raw[:5]])
    return raw
...
def search_summaries_by_vector(
    ...
    try:
        logger.debug("[search_summaries_by_vector] query=%r, app_id=%s, limit=%s", q[:80] if q else "", app_id, limit)
        helper = _get_text_helper()
        ...
        if not query_vec or len(query_vec) != SUMMARY_VEC_DIM:
            ...
            return []
        logger.debug("[search_summaries_by_vector] embedding ok, vec_dim=%s", len(query_vec))
        ...
        logger.debug("[search_summaries_by_vector] filter_query=%s", filter_query)
        ...
        if not results or not isinstance(results, list):
            logger.debug("[search_summaries_by_vector] no results: results=%s", type(results).__name__ if results is not None else "None")
            return []
        logger.debug("[search_summaries_by_vector] vector_search returned %s docs, top scores=%s", len(results), [round(r.get("score", 0), 3) for r in results[:5]])
        items = []
        for r in results:
            item = _doc_to_item(r)
