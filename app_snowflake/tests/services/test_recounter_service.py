...
class TestRecounterService(TransactionTestCase):
    ...
    def test_recount_transaction_rollback_on_exception(self):
        ...
        # 记录原始值
        original_recounter = Recounter.objects.using('snowflake_rw').get(dcid=self.datacenter_id, mid=self.machine_id)
        original_rc = original_recounter.rc
        original_ut = original_recounter.ut

        # 在一个会抛出异常的事务中调用 recount
        try:
            with transaction.atomic(using='snowflake_rw'):
                # 调用 recount 更新计数器
        ...
        recounter_after_rollback = Recounter.objects.using('snowflake_rw').get(dcid=self.datacenter_id, mid=self.machine_id)
