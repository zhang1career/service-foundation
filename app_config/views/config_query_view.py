...
class ConfigQueryView(APIView):
    """POST JSON: access_key, key, conditions (optional object)."""
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            cached = query_cache_get(reg_id, rid_gen, config_key, ch)
            if cached is not None:
                return response_with_request_id(request, resp_ok(cached), req_id)
            payload = merge_config_query_result(reg_id, config_key, conditions)
            query_cache_set(reg_id, rid_gen, config_key, ch, payload)
