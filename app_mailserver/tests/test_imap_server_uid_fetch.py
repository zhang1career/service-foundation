...
class TestIMAPServerUIDFetch(TransactionTestCase):
    ...
    def test_fetch_response_format_rfc3501_compliance(self):
        """测试 FETCH 响应格式符合 IMAP RFC 3501 标准"""
        # Create test message
        msg = MailMessage.objects.using('mailserver_rw').create(
            account_id=self.test_account.id,
            mailbox_id=self.test_mailbox.id,
        ...
        # According to RFC 3501, FETCH response format should be: * <sequence> FETCH (<data items>)
        import re
        pattern = r'^\* (\d+) FETCH \((.+)\)$'
        match = re.match(pattern, fetch_response)
        self.assertIsNotNone(match, f"FETCH response does not match RFC 3501 format: {fetch_response}")

        # Validate sequence number is positive integer
        sequence_num = int(match.group(1))
        self.assertGreaterEqual(sequence_num, 1, "Sequence number must be >= 1")

        # Validate data items (should contain UID and FLAGS)
        data_items = match.group(2)
        self.assertIn('UID', data_items, "Response should contain UID")
        self.assertIn('FLAGS', data_items, "Response should contain FLAGS")

        # Validate UID format: UID <integer>
        uid_pattern = r'UID\s+(\d+)'
        uid_match = re.search(uid_pattern, data_items)
        self.assertIsNotNone(uid_match, "UID should be followed by an integer")
        uid_value = int(uid_match.group(1))
        self.assertEqual(uid_value, msg.id, "UID should match message ID")

        # Validate FLAGS format: FLAGS (<flag list>) or FLAGS ()
        flags_pattern = r'FLAGS\s+(\([^)]*\))'
        flags_match = re.search(flags_pattern, data_items)
        self.assertIsNotNone(flags_match, "FLAGS should be in parenthesized format")
        flags_value = flags_match.group(1)
        self.assertTrue(flags_value.startswith('(') and flags_value.endswith(')'),
                       "FLAGS must be parenthesized")

    def test_fetch_response_sequence_number_validation(self):
        """测试 FETCH 响应中序列号验证 - 确保序列号始终是正整数"""
        ...
        handler = self._create_handler()
        uid_range = f"{min(msg1.id, msg2.id)}:{max(msg1.id, msg2.id)}"
        args = [uid_range, '(FLAGS)']
        responses, data_responses = asyncio.run(self._run_uid_fetch_test(handler, args))

        fetch_responses = self._filter_fetch_responses(responses)
        self.assertEqual(len(fetch_responses), 2)

        # Validate sequence numbers are positive integers and sequential
        import re
        sequence_numbers = []
        for fetch_response in fetch_responses:
            match = re.match(r'^\* (\d+) FETCH', fetch_response)
            self.assertIsNotNone(match, f"Invalid FETCH response format: {fetch_response}")
            seq = int(match.group(1))
            self.assertGreaterEqual(seq, 1, "Sequence number must be >= 1")
            sequence_numbers.append(seq)

        # Sequence numbers should be sequential (1, 2, ...)
        sequence_numbers.sort()
        self.assertEqual(sequence_numbers, [1, 2], "Sequence numbers should be sequential starting from 1")

    def test_fetch_response_data_items_separated_by_spaces(self):
        """测试 FETCH 响应中数据项之间用空格分隔（符合 RFC 3501）"""
        msg = MailMessage.objects.using('mailserver_rw').create(
            account_id=self.test_account.id,
            mailbox_id=self.test_mailbox.id,
            message_id='<test@example.com>',
        ...
        # Extract data items part (between parentheses)
        import re
        match = re.search(r'FETCH \((.+)\)$', fetch_response)
        self.assertIsNotNone(match)
        data_items = match.group(1)

        # According to RFC 3501, data items should be separated by spaces, not commas
        # Check that there are no commas between UID and FLAGS
        # Format should be: UID <n> FLAGS (...)
        self.assertNotIn(',', data_items, "Data items should be separated by spaces, not commas")

        # Validate format: UID <n> FLAGS (...)
        pattern = r'UID\s+\d+\s+FLAGS'
        match = re.search(pattern, data_items)
        self.assertIsNotNone(match, "UID and FLAGS should be separated by spaces")

    def test_fetch_response_uid_format_integer(self):
        """测试 FETCH 响应中 UID 格式为整数（符合 RFC 3501）"""
        msg = MailMessage.objects.using('mailserver_rw').create(
            account_id=self.test_account.id,
            mailbox_id=self.test_mailbox.id,
            message_id='<test@example.com>',
        ...
        # According to RFC 3501, UID should be formatted as: UID <integer>
        # Not as: UID "<integer>" or UID 'integer'
        import re
        uid_pattern = r'UID\s+(\d+)'
        uid_match = re.search(uid_pattern, fetch_response)
        self.assertIsNotNone(uid_match, "UID should be followed by an integer")
        
        uid_value = uid_match.group(1)
        # UID value should be numeric (no quotes, no string formatting)
        self.assertTrue(uid_value.isdigit(), f"UID value should be numeric, got: {uid_value}")
        
        # Verify UID value matches message ID
        self.assertEqual(int(uid_value), msg.id, "UID should match message ID")

        # Verify UID is not quoted
        quoted_pattern = r'UID\s+["\']'
        self.assertNotRegex(fetch_response, quoted_pattern, "UID should not be quoted")

    def test_fetch_response_flags_parenthesized_format(self):
        """测试 FETCH 响应中 FLAGS 格式为括号包裹的列表（符合 RFC 3501）"""
        # Test with flags
        msg = MailMessage.objects.using('mailserver_rw').create(
            account_id=self.test_account.id,
            mailbox_id=self.test_mailbox.id,
        ...
        # According to RFC 3501, FLAGS must always be parenthesized
        # Format: FLAGS (\Seen) or FLAGS (\Seen \Flagged) or FLAGS ()
        import re
        flags_pattern = r'FLAGS\s+(\([^)]*\))'
        flags_match = re.search(flags_pattern, fetch_response)
        self.assertIsNotNone(flags_match, "FLAGS should be in parenthesized format")
        
        flags_value = flags_match.group(1)
        self.assertTrue(flags_value.startswith('(') and flags_value.endswith(')'),
                       f"FLAGS must be parenthesized, got: {flags_value}")

        # Test with no flags (empty mailbox or no flags set)
        msg2 = MailMessage.objects.using('mailserver_rw').create(
            account_id=self.test_account.id,
            mailbox_id=self.test_mailbox.id,
        ...
        args2 = [str(msg2.id), '(FLAGS)']
        responses2, _ = asyncio.run(self._run_uid_fetch_test(handler, args2))

        fetch_responses2 = self._filter_fetch_responses(responses2)
        self.assertGreater(len(fetch_responses2), 0)

        fetch_response2 = fetch_responses2[0]

        # Even with no flags, FLAGS should be parenthesized: FLAGS ()
        flags_match2 = re.search(flags_pattern, fetch_response2)
        self.assertIsNotNone(flags_match2, "FLAGS should be in parenthesized format even with no flags")
        
        flags_value2 = flags_match2.group(1)
        self.assertEqual(flags_value2, '()', "FLAGS should be () when no flags are set")

    def test_fetch_response_php_client_compatibility(self):
        """测试 FETCH 响应格式与 PHP IMAP 客户端兼容性 - 确保 $tokens[1] 始终存在且为 'FETCH'"""
        msg = MailMessage.objects.using('mailserver_rw').create(
            account_id=self.test_account.id,
            mailbox_id=self.test_mailbox.id,
            message_id='<test@example.com>',
        ...
        # Simulate PHP IMAP client parsing logic
        # PHP client uses decodeLine() to parse responses into $tokens array
        # Format: * <sequence> FETCH (<data items>)
        # After parsing: $tokens[0] = '*', $tokens[1] = '<sequence>', $tokens[2] = 'FETCH', ...
        
        # Verify response format ensures $tokens[1] exists
        import re
        pattern = r'^\* (\d+) FETCH'
        match = re.match(pattern, fetch_response)
        self.assertIsNotNone(match, "Response must match format: * <sequence> FETCH ...")
        
        # Verify FETCH is at expected position (third token, index 2)
        parts = fetch_response.split()
        self.assertGreaterEqual(len(parts), 3, "Response must have at least 3 parts")
        self.assertEqual(parts[0], '*', "First token must be '*'")
        self.assertTrue(parts[1].isdigit(), "Second token must be sequence number")
        self.assertEqual(parts[2], 'FETCH', "Third token must be 'FETCH' (ensures $tokens[1] exists in PHP)")

    def test_fetch_response_sequence_number_edge_cases(self):
        """测试序列号边界情况 - None、0、负数等"""
        msg = MailMessage.objects.using('mailserver_rw').create(
            account_id=self.test_account.id,
            mailbox_id=self.test_mailbox.id,
            message_id='<test@example.com>',
        ...
        # Even if message.id was None or invalid, response should have valid sequence number
        import re
        match = re.match(r'^\* (\d+) FETCH', fetch_response)
        self.assertIsNotNone(match, "Response must have valid sequence number")
        
        sequence_num = int(match.group(1))
        self.assertGreaterEqual(sequence_num, 1, "Sequence number must be >= 1 even with edge cases")

    def test_fetch_response_all_data_item_types_consistency(self):
        """测试所有 FETCH 数据项类型的响应格式一致性"""
        msg = MailMessage.objects.using('mailserver_rw').create(
            account_id=self.test_account.id,
            mailbox_id=self.test_mailbox.id,
            message_id='<test@example.com>',
        ...
        for data_items in data_item_types:
            args = [str(msg.id), data_items]
            responses, _ = asyncio.run(self._run_uid_fetch_test(handler, args))
            
            fetch_responses = self._filter_fetch_responses(responses)
            self.assertGreater(len(fetch_responses), 0, 
                             f"Should have FETCH response for data items: {data_items}")
            
            fetch_response = fetch_responses[0]
            
            # All responses should follow RFC 3501 format: * <sequence> FETCH (<data items>)
            import re
            pattern = r'^\* (\d+) FETCH \((.+)\)$'
            match = re.match(pattern, fetch_response)
            self.assertIsNotNone(match, 
                               f"FETCH response must match RFC 3501 format for {data_items}: {fetch_response}")
            
            # Validate sequence number
            sequence_num = int(match.group(1))
            self.assertGreaterEqual(sequence_num, 1, 
                                  f"Sequence number must be >= 1 for {data_items}")

    def test_fetch_response_exception_handling(self):
        """测试异常处理 - 当 send_fetch_response 发生异常时不应发送格式不正确的响应"""
        # This test verifies that exceptions in send_fetch_response don't result in malformed responses
        msg = MailMessage.objects.using('mailserver_rw').create(
            account_id=self.test_account.id,
            mailbox_id=self.test_mailbox.id,
            message_id='<test@example.com>',
        ...
        # Check that there are no malformed FETCH responses
        fetch_responses = self._filter_fetch_responses(responses)
        for fetch_response in fetch_responses:
            # Verify each FETCH response is well-formed
            import re
            pattern = r'^\* (\d+) FETCH \((.+)\)$'
            match = re.match(pattern, fetch_response)
            self.assertIsNotNone(match, 
                               f"All FETCH responses must be well-formed: {fetch_response}")
            
            # Verify sequence number is valid
            sequence_num = int(match.group(1))
            self.assertGreaterEqual(sequence_num, 1, 
                                  f"Sequence number must be valid: {fetch_response}")

        # Should have OK response
        ok_responses = [r for r in responses if 'OK UID FETCH completed' in r]
        self.assertEqual(len(ok_responses), 1, "Should have OK response")
