...
class TestOSSIntegrationService(TestCase):
    ...
    @patch('app_mailserver.services.oss_integration.get_app_config')
    @patch('app_mailserver.services.oss_integration.boto3')
    ...
    def test_get_oss_service_singleton(self, mock_boto3, mock_get_config):
        ...
        # 清除之前的单例实例（如果存在）
        import app_mailserver.services.oss_integration
        if hasattr(app_mailserver.services.oss_integration, '_oss_service'):
            app_mailserver.services.oss_integration._oss_service = None
        
        service1 = get_oss_service()
        service2 = get_oss_service()
