...
def create_reg(name: str, status: int = 0) -> Reg:
    ...
    return Reg.objects.using("aibroker_rw").create(
        ...
        callback_secret=uuid.uuid4().hex + uuid.uuid4().hex,
        status=status,
        ct=now_ms,
        ut=now_ms,
    )
...
def list_regs():
    return list(Reg.objects.using("aibroker_rw").all().order_by("-id"))


def get_reg_by_id(reg_id: int) -> Optional[Reg]:
    return Reg.objects.using("aibroker_rw").filter(id=reg_id).first()


def get_reg_by_access_key(access_key: str) -> Optional[Reg]:
    return Reg.objects.using("aibroker_rw").filter(access_key=access_key).first()
...
def delete_reg(reg_id: int) -> bool:
    deleted, _ = Reg.objects.using("aibroker_rw").filter(id=reg_id).delete()
    return deleted > 0
