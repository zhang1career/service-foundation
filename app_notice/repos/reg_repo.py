...
def list_regs():
    return list(Reg.objects.using("notice_rw").all().order_by("-id"))


def get_reg_by_id(reg_id: int) -> Optional[Reg]:
    return Reg.objects.using("notice_rw").filter(id=reg_id).first()


def get_reg_by_access_key(access_key: str) -> Optional[Reg]:
    return Reg.objects.using("notice_rw").filter(access_key=access_key).first()
...
def delete_reg(reg_id: int) -> bool:
    deleted, _ = Reg.objects.using("notice_rw").filter(id=reg_id).delete()
    return deleted > 0
